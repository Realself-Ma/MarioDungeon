...
string MysqlServer::ChatRoomMessage(const TcpConnectionPtr& conn,string& msg,const string& playerName)
{
	...
	int size=msg.size();
	//cout<<"msg:"<<msg<<" size:"<<size<<endl;
	char msgLen[10];
	snprintf(msgLen,sizeof(msgLen),"/%d/",size);
	string s(msgLen);
	msg=s+msg;
	//cout<<"msg:"<<msg<<" size:"<<msg.size()<<endl;
...
{	
	...
	if(strcmp(sqlrow[0],"1")==0)
		...
		rq="玩家未准备!";
	ChatRoomMessage(conn,rq,playername);
	return "";
}
...
string MysqlServer::mapRequest(const TcpConnectionPtr& conn,char* playername)
{
	...
	msg=s+msg;
	*/
	ChatRoomMessage(conn,msg,playername);
	return "";
}
